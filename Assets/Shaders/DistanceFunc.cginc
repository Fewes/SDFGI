...
SDFO SDFObject (float3 P, float ID, float3 ALBEDO, float SPECULAR, float ROUGHNESS)
{
	SDFO sdfo;
	sdfo.p = P;
	sdfo.id = ID;
	sdfo.albedo = ALBEDO;
	sdfo.specular = SPECULAR;
	sdfo.roughness = ROUGHNESS;

	return sdfo;
}

SDFO opU(SDFO d1, SDFO d2)
{
	if (d1.p < d2.p)
		return d1;
	else
...
SDFO opS(SDFO d2, SDFO d1)
{
	if (-d1.p > d2.p)
	{
		...
		d1.id = d2.id;
		d1.albedo = d2.albedo;
		d1.specular = d2.specular;
		d1.roughness = d2.roughness;
		return d1;
	}
	else
		return d2;
